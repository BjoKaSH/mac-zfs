...
function tests_std_setup() {
    ...
    if [ "${1:-}" == "-C" ] ; then
        ...
        echo "$0 [ -C conf-file ]  [ --diskstore dir ]  [ --poolbase prefix ]  [ --temp dir ]  [ --logdir dir ]  [ --genrand path-to-genrand ]    ${extra_args_help_short:-}"
        echo "   -C conf-file must be first argument, if present."
        echo "   --diskstore  empty directory used to store disk images"
        echo "   --poolbase   prefix for pool names generated by the test system"
    ...
    if [ ! -x "${genrand_bin}" ] ; then
        ...
        if [ ! -f "${test_scripts_dir}/genrand.c" ] ; then
            echo "source file 'genrand.c' for random number geerator not found. Giving up."
            exit 1
        fi
        gcc -o "${test_scripts_dir}/genrand" -O3 "${test_scripts_dir}/genrand.c" || exit 1
        genrand_bin="${test_scripts_dir}/genrand"
    fi
